...
class GameData {
  ...
  async initDatabase() {
    ...
    await this.db.run(`
            CREATE TABLE IF NOT EXISTS game_state (
                ...
                guard_protect TEXT NOT NULL DEFAULT 'null',
                idiot_act_dumb TEXT DEFAULT NULL
            )
        `);
    await this.db.run(`
  ...
  async addVote(groupId, userId, targetId) {
    try {
      await this.db.run(
        "INSERT OR REPLACE INTO votes (group_id, user_id, target_id, vote_time) VALUES (?, ?, ?, ?)",
        [groupId, userId, targetId, Date.now()],
      );
      logger.mark(`User ${userId} voted for ${targetId} in group ${groupId}.`);
      return true;
    } catch (err) {
      logger.error("Failed to add vote:", err.message);
      return false;
    }
  }
  ...
  async getVotes(groupId) {
    try {
      const rows = await this.db.all(
        "SELECT user_id, target_id FROM votes WHERE group_id = ?",
        [groupId],
      );
      return rows;
    } catch (err) {
      logger.error("Failed to get votes:", err.message);
      return [];
    }
  }

  async clearVotes(groupId) {
    try {
      const result = await this.db.run("DELETE FROM votes WHERE group_id = ?", [
        groupId,
      ]);
      logger.mark(`Votes cleared for group ${groupId}.`);
      return result.changes;
    } catch (err) {
      logger.error("Failed to clear votes:", err.message);
      return false;
    }
  }
    ...
    try {
      ...
      if (row) {
        return {
          ...
          guardProtect: row.guard_protect === "null" ? null : row.guard_protect,
          idiotActDumb: row.idiot_act_dumb,
        };
      } else {
        return {
          witch: {
  ...
  async setGame(groupId, game) {
    try {
      await this.db.run(
        "INSERT OR REPLACE INTO game_state (group_id, witch_save, witch_poison, kill_list, guard_protect, idiot_act_dumb) VALUES (?, ?, ?, ?, ?, ?)",
        [
          groupId,
          game.witch.save ? 1 : 0,
